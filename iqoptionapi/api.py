...
class IQOptionAPI(object):  # pylint: disable=too-many-instance-attributes
    ...
    @property
    def verify(self):
        """Property for get IQ Option http verify resource.
...
# -------------------------------------------------------
        ...
        while True:
            try:
                ...
                if time.time() - start_time > max_wait_time:
                    ...
                    return False, "Timeout waiting for websocket connection"
    ...
    def send_ssid(self):
        ...
        while self.profile.msg==None:
            ...
            if time.time() - start_time > max_wait_time:
                logger = logging.getLogger(__name__)
                logger.error("Timeout waiting for profile message")
                return False
    ...
    def connect(self):
        ...
        if global_value.SSID!=None:
            ...
            if check_ssid==False:
                ...
                try:
                    ...
                    logger = logging.getLogger(__name__)
                    logger.error(f"Failed to get SSID from response cookies: {e}")
        ...
        while True:
            try:
                ...
                if time.time() - start_time > max_wait_time:
                    logger = logging.getLogger(__name__)
                    logger.error("Timeout waiting for server timestamp")
                    return False, "Timeout waiting for server timestamp"

            except AttributeError as e:
                logger = logging.getLogger(__name__)
                logger.error(f"Error checking server timestamp: {e}")
