...
class IQ_Option:
    ...
    def connect(self):
        ...
        if check == False and isinstance(reason, str):
            try:
                # Parse the JSON response
                import json
                verification_data = json.loads(reason)
                if verification_data.get("code") == "verify" and verification_data.get("method") == "sms":
                    logging.warning("SMS verification required. Please check your phone for a verification code.")
                    print("\n=== SMS Verification Required ===")
                    print("IQ Option now requires SMS verification for your account.")
                    ...
                    otp_code = input("Verification code: ")

                    # Submit the OTP code
                    logging.info("Submitting verification code...")
                    verify_response = self.api.verify(token, otp_code)
                    ...
                    if verify_response.status_code == 200:
                        ...
                        logging.error(f"Verification failed: {verify_response.text}")
                        return False, verify_response.text
                ...
                if "verify" in reason:
                    ...
                    # Try to extract token from the error message
                    import re
                    ...
                    if token_match:
                        ...
                        try:
                            ...
                            logging.error(f"Error submitting verification code: {str(ve)}")
                            return False, str(ve)
                    else:
                        logging.error("Could not extract verification token from response")
                        return False, reason
        ...
        if check == True:
            ...
            max_wait_time = 30  # seconds - increased from 10 to 30
            start_time = time.time()

            while global_value.balance_id == None:
                # Check if we've waited too long
                if time.time() - start_time > max_wait_time:
                    logging.error("Timeout waiting for balance_id")
    ...
    def get_all_open_time(self):
        ...
        for option in binary_list:
            # Handle 'blitz' as an alias for 'turbo'
            option_key = "turbo" if option == "blitz" else option
            for actives_id in binary_data[option_key]["actives"]:
                active = binary_data[option_key]["actives"][actives_id]
        ...
        if digital_data and "underlying" in digital_data:
            digital_list = digital_data["underlying"]
        else:
            # If 'underlying' key is missing, use the data directly
            digital_list = digital_data if digital_data else []
    ...
    def get_profile_ansyc(self):
        ...
        while self.api.profile.msg == None:
            ...
            if time.time() - start_time > max_wait_time:
                ...
                logger.error("Timeout waiting for profile data in get_profile_ansyc")
    ...
    def get_currency(self):
        try:
            ...
            if balances_raw and "msg" in balances_raw:
                ...
                logger = logging.getLogger(__name__)
                logger.error("Invalid balances data: missing msg")
                return None
        except TimeoutError as te:
            logger = logging.getLogger(__name__)
            logger.error(f"Timeout getting balances data: {te}")
            return None
        except Exception as e:
            logger = logging.getLogger(__name__)
            logger.error(f"Error processing balances data: {e}")
            return None
    ...
    def get_balances(self):
        ...
        while self.api.balances_raw == None:
            ...
            if time.time() - start_time > max_wait_time:
                logger = logging.getLogger(__name__)
                logger.error("Timeout waiting for balances data in get_balances")
                raise TimeoutError("Timeout waiting for balances data")
            time.sleep(0.1)  # Avoid busy waiting
    ...
    def get_balance_mode(self):
        ...
        try:
            ...
            if profile and "balances" in profile:
                ...
                logger = logging.getLogger(__name__)
                logger.error("Invalid profile data: missing balances")
                return None
        except TimeoutError as te:
            logger = logging.getLogger(__name__)
            logger.error(f"Timeout getting profile data: {te}")
            return None
        except Exception as e:
            logger = logging.getLogger(__name__)
    ...
    def reset_practice_balance(self):
        ...
        while self.api.training_balance_reset_request == None:
            ...
            if time.time() - start_time > max_wait_time:
                logger = logging.getLogger(__name__)
                logger.error("Timeout waiting for training balance reset request")
                raise TimeoutError("Timeout waiting for training balance reset request")
            time.sleep(0.1)  # Avoid busy waiting
    ...
    def get_candles(self, ACTIVES, interval, count, endtime):
        ...
        # Add timeout mechanism for the outer loop
        max_attempts = 3
        attempt = 0
        ...
        while attempt < max_attempts:
            ...
            try:
                ...
                # Add timeout mechanism for the inner loop
                max_wait_time = 10  # seconds
                start_time = time.time()
                ...
                while self.check_connect and self.api.candles.candles_data == None:
                    ...
                    if time.time() - start_time > max_wait_time:
                        logger = logging.getLogger(__name__)
                        logger.error(f"Timeout waiting for candles data (attempt {attempt}/{max_attempts})")
                        break  # Break the inner loop and try again
                    time.sleep(0.1)  # Avoid busy waiting
                ...
                if attempt >= max_attempts:
                    logger = logging.getLogger(__name__)
                    logger.error(f"Failed to get candles data after {max_attempts} attempts")
                    raise TimeoutError("Timeout waiting for candles data")
    ...
    def buy_multi(self, price, ACTIVES, ACTION, expirations):
        ...
        if len(price) == len(ACTIVES) == len(ACTION) == len(expirations):
            ...
            # Get open time information once for all assets
            open_time = self.get_all_open_time()

            # Process each asset before placing trades
            processed_actives = []
            for idx in range(buy_len):
                active = ACTIVES[idx]

                # Check if active is available in binary/turbo markets
                is_regular_available = False
                is_otc_available = False

                # Check if regular version is available in binary or turbo
                if active in open_time["binary"] and open_time["binary"][active]["open"]:
                    is_regular_available = True
                elif active in open_time["turbo"] and open_time["turbo"][active]["open"]:
                    is_regular_available = True

                # Check if OTC version is available in binary or turbo
                otc_active = f"{active}-OTC"
                if otc_active in open_time["binary"] and open_time["binary"][otc_active]["open"]:
                    is_otc_available = True
                elif otc_active in open_time["turbo"] and open_time["turbo"][otc_active]["open"]:
                    is_otc_available = True

                # If regular version is not available but OTC is, switch to OTC
                if not is_regular_available and is_otc_available:
                    logging.info(f"Switching from {active} to {otc_active} as regular version is not available")
                    processed_actives.append(otc_active)
                else:
                    processed_actives.append(active)

            # Now place all trades with processed actives
            for idx in range(buy_len):
                self.api.buyv3(
                    price[idx], OP_code.ACTIVES[processed_actives[idx]], ACTION[idx], expirations[idx], idx)
    ...
    def get_remaning(self, duration):
        if self.api.timesync.server_timestamp is None:
            logger = logging.getLogger(__name__)
            logger.error("Server timestamp is None in get_remaning")
            return "ERROR server timestamp is None"
    ...
    def buy_by_raw_expirations(self, price, active, direction, option, expired):
        ...
        # If regular version is not available but OTC is, switch to OTC
        if not is_regular_available and is_otc_available and active == "EURUSD":
            logging.info(f"Switching from {active} to {otc_active} as regular version is not available")
            active = otc_active
    ...
    def buy(self, price, ACTIVES, ACTION, expirations):
        ...
        # Check if ACTIVES is available in binary/turbo markets
        open_time = self.get_all_open_time()
        is_regular_available = False
        is_otc_available = False

        # Check if regular version is available in binary or turbo
        if ACTIVES in open_time["binary"] and open_time["binary"][ACTIVES]["open"]:
            is_regular_available = True
        elif ACTIVES in open_time["turbo"] and open_time["turbo"][ACTIVES]["open"]:
            is_regular_available = True

        # Check if OTC version is available in binary or turbo
        otc_active = f"{ACTIVES}-OTC"
        if otc_active in open_time["binary"] and open_time["binary"][otc_active]["open"]:
            is_otc_available = True
        elif otc_active in open_time["turbo"] and open_time["turbo"][otc_active]["open"]:
            is_otc_available = True
        ...
        # If regular version is not available but OTC is, switch to OTC
        if not is_regular_available and is_otc_available and ACTIVES == "EURUSD":
            logging.info(f"Switching from {ACTIVES} to {otc_active} as regular version is not available")
            ACTIVES = otc_active
    ...
    def get_digital_underlying_list_data(self):
        ...
        while self.api.underlying_list_data == None:
            if time.time() - start_t >= 30:
                ...
                logging.info('Falling back to regular binary options trading')
                # Return a special dictionary with a flag indicating to use binary options
                return {"use_binary_options": True}
    ...
    def buy_digital_spot(self, active, amount, action, duration):
        ...
        # Check if digital underlying list data is available
        digital_list = self.get_digital_underlying_list_data()
        if digital_list is not None and isinstance(digital_list, dict) and digital_list.get("use_binary_options", False):
            # Digital options are not available, fall back to regular binary options
            logging.info(f"Falling back to regular binary options for {active}")
            binary_action = "put" if action == "P" else "call"
            return self.buy(amount, active, binary_action, duration)

        # Check if active is available in digital market
        open_time = self.get_all_open_time()
        is_regular_available = False
        is_otc_available = False

        # Check if regular version is available in digital
        if active in open_time["digital"] and open_time["digital"][active]["open"]:
            is_regular_available = True

        # Check if OTC version is available in digital
        otc_active = f"{active}-OTC"
        if otc_active in open_time["digital"] and open_time["digital"][otc_active]["open"]:
            is_otc_available = True
